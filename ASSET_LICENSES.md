# Asset Licenses

## Brand and UI artwork

| Asset                                                      | Source                                    | License                        | Notes                                  |
| ---------------------------------------------------------- | ----------------------------------------- | ------------------------------ | -------------------------------------- |
| `public/favicon.svg`                                       | Original — Che Xu Studio / NorthLine demo | All rights reserved (demo use) | Custom NL mark                         |
| `public/images/og-northline.svg`                           | Original — Che Xu Studio / NorthLine demo | All rights reserved (demo use) | Social sharing image                   |
| Inline hero SVG on homepage                                | Original — Che Xu Studio / NorthLine demo | All rights reserved (demo use) | Decorative illustration                |
| Icon set in `src/components/icons` and `ServiceIcon.astro` | Original stroke icons                     | All rights reserved (demo use) | Not copied from a commercial icon pack |

## Fonts

| Asset                   | Source                                                                      | License                   |
| ----------------------- | --------------------------------------------------------------------------- | ------------------------- |
| DM Sans Variable        | `@fontsource-variable/dm-sans` (upstream: Google Fonts / Fontsource)        | SIL Open Font License 1.1 |
| Source Serif 4 Variable | `@fontsource-variable/source-serif-4` (upstream: Google Fonts / Fontsource) | SIL Open Font License 1.1 |

## Third-party runtime assets

| Asset                       | Source                                                  | License / Terms                                             |
| --------------------------- | ------------------------------------------------------- | ----------------------------------------------------------- |
| Cloudflare Turnstile script | `https://challenges.cloudflare.com/turnstile/v0/api.js` | Cloudflare terms; loaded only when a site key is configured |

## Intentionally unused

- No scraped stock photographs
- No hotlinked third-party images
- No autoplay video
- No fabricated review widgets
