/**
 * Capture recommended case-study tablet screenshots (768×1024).
 * Mirrors CASE_STUDY_COPY.md → Screenshots, framed for tablet.
 *
 * At 768px the site still uses hamburger + sticky action bar
 * (desktop nav/CTAs appear at 1024px+).
 */
...
// ---------------------------------------------------------------------------
// 2. Sticky action bar (still present on tablet < 1024px)
// ---------------------------------------------------------------------------
await gotoPrepared(page, '/');
...
await writeElementShot(page.locator('.mobile-action-bar'), '02b-tablet-action-bar-detail.png');
...
await page.evaluate(() => {
  document.querySelector('.request-flow__actions .btn-primary')?.scrollIntoView({
    block: 'center',
  });
});
...
const recommended = [
  ...
  ['02-tablet-sticky-action-bar.png', '2-tablet-sticky-action-bar.png'],
  ['02b-tablet-action-bar-detail.png', '2b-tablet-action-bar-detail.png'],
...
for (const [src, dest] of recommended) {
  await copyFile(path.join(REPO_DIR, src), path.join(RECOMMENDED_DIR, dest)).catch(() => {});
}
